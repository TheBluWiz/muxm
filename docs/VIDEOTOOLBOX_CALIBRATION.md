...
- **No Dolby Vision passthrough test.** Calibration clips include DV Profile 7 RPU side data, but the DV layer is not evaluated — VT encodes produce HDR10-only output; DV injection happens post-hoc via `dovi_tool` (encoder-agnostic). VT + DV compatibility is tracked separately.
